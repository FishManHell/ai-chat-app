...
const HomePage = () => {
  return (
    <div className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden bg-linear-to-br from-[#0F0B1E] to-[#1A1040] px-4">
      <GlowBlob className="-top-40 right-20 h-150 w-150 opacity-15" color="var(--brand-mid)" />
      <GlowBlob className="-bottom-40 left-20 h-150 w-150 opacity-20" color="var(--brand-start)" />
      <GlowBlob className="top-1/2 left-1/2 h-100 w-100 -translate-x-1/2 -translate-y-1/2 opacity-10" color="var(--brand-end)" />

      <div className="relative z-10 flex flex-col items-center gap-8 text-center">
        <div
          className="h-20 w-20 rounded-full bg-linear-to-br from-(--brand-start) via-(--brand-mid) to-(--brand-end) shadow-2xl"
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <Link
            ...
            className={cn(
              ...
              "text-white font-semibold shadow-(--glow-primary)",
